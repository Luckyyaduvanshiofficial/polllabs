...
async def get_poll_badge(poll_id: str, pb: PocketBaseDep) -> Response:
    """
    Renders a live SVG badge for GitHub READMEs (PRD §4.4, §4.6).
    Shows a graceful 'This poll is no longer available' state when a poll is missing or deleted.
    """
    poll = await pb.get_poll(poll_id)
    if not poll:
        # PRD §4.6: Graceful "This poll is no longer available" state
        svg_content = render_svg_badge("poll", "no longer available", is_error=True)
    else:
